...
```
1. Typeless プロセスが見つかる → osascript -e 'tell application "System Events"
   to key code 79' で F18 を emit し、Typeless 内グローバルホットキー (F18 にバインド済) を
   トリガー。Caps Lock は Karabiner が voice-toggle に変換するので、Typeless に Caps Lock
   を直接登録しても届かない。F18 を仲介キーとして使うことで、Caps Lock ⇄ Handy/Typeless
   どちらでも同じ物理キーから録音できる。
2. Handy プロセス (binary 名 `handy` 小文字) が見つかる → handy --toggle-post-process
3. どちらも起動していない → open -a Handy (既存挙動維持)
```

プロセス検出は Handy が `pgrep -x handy`、Typeless は bundle path 経由
`pgrep -f /Applications/Typeless.app/Contents/MacOS/` を使う。Typeless の binary 名は
Electron 系で揺れる可能性があるため、bundle path で確実に捕捉する。

Typeless 側の F18 登録は GUI 操作 1 回 (録音ホットキー設定欄にフォーカスを当てた状態で
`osascript -e 'tell application "System Events" to key code 79'` を実行) で完結する。
物理 F18 キーは標準キーボードに無いが、System Events 経由で emit したイベントを Typeless が
受信して登録する。
