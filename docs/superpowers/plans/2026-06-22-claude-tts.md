...
- ボイス: `Kyoko`、話速: `220 wpm` を初期値とし、`CLAUDE_TTS_VOICE` / `CLAUDE_TTS_RATE` 環境変数で上書き可能
- 暴走上限: 50,000 文字でカット
...
printf '%s' "$INPUT" | /usr/bin/jq -r '.transcript_path // empty' \
...
```bash
/usr/bin/jq '.profiles[0].complex_modifications.rules | map(.description)' /Users/snakashima/dotfiles/karabiner/karabiner.json
```
...
```bash
/usr/bin/jq '.' /Users/snakashima/dotfiles/karabiner/karabiner.json > /dev/null && echo "JSON OK"
```
...
#   CLAUDE_TTS_RATE    default: 220 wpm
