...
その日の作業を1コマンドで締める。**Slack+GitHub収集 → github-issues（open issue 確認）→ daily-log → CloudLog入力 → generate-problem → 翌日デイリー作成** を順次実行する。

デフォルトで `--exclude siori --exclude generate-video --exclude shindanshi --exclude microsoft-agent-hackathon --exclude kunstSite` を適用する。`--exclude` 追加指定があれば合算する。
...
```
/daily-log --session --cloudlog --exclude siori --exclude generate-video --exclude shindanshi --exclude microsoft-agent-hackathon --exclude kunstSite [追加 --exclude ...]
```
