...
description: >-
  ...
  Slack+GitHub 収集 → open issue 確認 → 日報生成 → CloudLog 入力 → 振り返り → 英語学習ログ → 北極星ロースト更新 → 翌日デイリー作成 → 翌日タスクの GitHub issue 紐付け → Obsidian vault commit/push を順次実行する。
  除外プロジェクト指定は本文の Options を参照。
argument-hint: "[--exclude <キーワード>...]"
user-invocable: true
---

その日の作業を1コマンドで締める。**Slack+GitHub収集 → github-issues（open issue 確認）→ daily-log → CloudLog入力 → generate-problem → 北極星ロースト更新 → 翌日デイリー作成 → 翌日タスク整理（GitHub issue 紐付け）→ Obsidian vault commit/push** を順次実行する。
...
### Step 5.2: english-log（英語学習ログ）

`english-log` skill に従い、当日セッションで Claude が与えた英語訂正（"X" → "Y"）と英訳例（In English:）を vault の `02_skillup/english/YYYY-MM-DD.md` に追記する。

- 素材はこのセッション（および走査可能な当日セッション）の訂正・英訳例。**材料が 1 件もなければ「english-log: 該当なし」と完了報告に記録してスキップ**する（エラー扱いしない）
- Step 0 のスキップ対象には含めない軽量ステップだが、`Other` でのスキップ指定は受け付ける
...
Step 6 の翌日デイリー（既に存在していた場合も対象）の `## 今日やること` セクションにタスクを書き込む。各タスクは GitHub issue と紐付け、リンクをタスク行に差し込む。

**タスクの受け取り:**
1. ユーザーに「明日やるタスク」を尋ねる。参考情報として以下を併せて提示する:
   - Step 2 で取得済みの open issue 一覧（cmb-sy assigned。再取得しない。Step 2 が実行されず一覧が無い場合のみ、ここで `/github-issues` の list を実行して取得する）
   - 本日の日報の `## 今日やること` に `- [ ]` のまま残っているタスク（繰り越し候補）
2. 「なし」「スキップ」の回答なら本ステップを終了し、完了報告に「翌日タスク: なし」と記録する
...
- タスク行は `- メイン` / `- 雑務（息抜き, Mtg）` の配下に、テンプレートと同じインデント（タブ 1 つ）で書く。空の `- [ ] ` プレースホルダ行は最初のタスク行で置き換え、2 件目以降は同じインデントで受け取り順に行を追加する
...
- リンクなしのタスク行: `- [ ] {タスク内容}`
- 「メイン」「雑務（息抜き, Mtg）」の振り分けはタスク内容から判断し、迷う場合はタスク受け取り時にまとめて確認する
- テンプレート由来の行（`[[01_quant/キャリア]]を確認`・`[Githubタスク](...)`・`- メイン` / `- 雑務` の行）は変更しない
- 翌日デイリーが既存でタスク行が既に書かれている場合は、既存行の文言を保持したまま issue リンクの差し込みのみ行う（issue 未紐付けの既存行も手順 3〜6 の対象に含める）
