...
- **1 スライド 1 メッセージ**: メッセージ = 聴衆に取ってほしい判断・思考。テーマ・章立て・カテゴリ名は主張ではない。「ダッシュボード」「サマリ」「全 KPI 一覧」を 1 メッセージと呼ばない。1 枚に詰め込みたくなったら、それは設計の失敗。スライドを分けるか、情報を捨てる
...
- **色は乱用しない**: 基本はテンプレ標準色（テキスト・タイトル・アクセント）の 3 色まで。アクセント色は 1 スライドに 1〜2 箇所、強調すべき数値・キーワードのみに使う。bullets を色分けで意味付けしない（順序・重要度はレイアウトで表現）。**色をカテゴリ分類のエンコーディングに使わない**（5 カテゴリを 5 色で塗り分けたくなったら、ラベル・グループ枠線・空白セパレータで表現する）
...
- **本文 14pt / タイトル 24pt 未満のフォントを使わない**: スペースに収まらないなら、文字を削るか layout を分ける。フォントを縮めて詰め込むのは禁止
- **audit required ゲートを省略しない**: Phase 3（Storyline Review）/ Phase 5（Slide Plan Review）/ Phase 8（Visual Audit）は必須。「1 人プレゼン」「時間がない」「品質ゲートは過剰」「実験的だから」は理由にならない
...
- **原則違反の最終判断をユーザー・第三者に委譲しない**: 「妥当性は部長の最終判断に委ねる」式の責任回避はしない。プロとして「この設計はこの原則に反するため不採用」を自分で決定し、その上で代案を提示する
- **「audit ゲートを省略していい？」と聞かれたら拒否する**: 代わりに省略可能な代替を提示する — Codex (`--codex` を外す)、iterations を 1 に下げる、subagent 並列を直列に変える、テンプレのアクセント色を 1 つに減らす など。Phase 3 / 5 / 8 の audit 本体は省略不可
