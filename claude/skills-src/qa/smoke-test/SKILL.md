...
name: smoke-test
description: >-
  変更を実ブラウザで動かして検証するローカルスモークテスト。dev サーバー起動 →
  アドホックテスト生成・実行 → VRT 差分チェック → E2E 実行 + フレーキー検出を実行する。
  実装完了後の動作確認、feature-dev / debug-flow の smoke フェーズから invoke される。
  単体起動も可能。browser-use CLI による UI 操作検証であり、ユニット/統合テストの代替ではない。
user-invocable: true
---

# Smoke Test

実装した機能の動作確認・VRT 差分チェック・E2E フレーキー検出を browser-use CLI で自律的に実行する。browser-use CLI の導入確認は Step 2 前提条件チェックで行う。
...
browser-use CLI を使い、各シナリオを検証する。
...
**シナリオ失敗時のリトライ手順（1 シナリオあたり）:**

1. 初回実行が FAIL の場合、シナリオ記述を見直して修正する（リトライ 1 回目を実行）
2. リトライ 1 回目も FAIL の場合、さらに修正して再実行する（リトライ 2 回目を実行）
3. リトライ 2 回目も FAIL の場合、そのシナリオを FAIL 確定とし、以降リトライしない

すなわち初回 1 回 + リトライ最大 2 回 = 1 シナリオあたり最大 3 実行。ただしアプリケーションバグが原因と判明した時点でリトライを打ち切り即 FAIL 確定とする（同じ実装バグを 3 回叩かない）。
...
| 2 | browser-use CLI 実行エラー | 修正 → 再実行（初回 + リトライ最大2回） |
