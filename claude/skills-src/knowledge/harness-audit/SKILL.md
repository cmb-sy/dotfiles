...
description: >-
  ...
  欠陥・不足・構築手順を教育的に報告、docs/harness/ にレポート保存する。修正は実行しない(診断・設計・教育に徹する)。
...
- 前回レポート(`docs/harness/*.md` の最新)があれば読み、**次元ごとのスコア推移**を算出する
...
2. `docs/harness/YYYY-MM-DD-harness-audit.md` に全文保存(ディレクトリなければ作成、同日は連番):
   frontmatter(date / scores / prev_scores) → スコアカード → evidence → 教育解説 → アーキテクチャ → ロードマップ → 参照 URL
3. 前回レポートがある場合、冒頭にスコア推移表(前回→今回)を必ず載せる
