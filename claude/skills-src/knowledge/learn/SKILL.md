...
- header: `保存先`
- 選択肢:
  1. **プロジェクト（推奨）** — `docs/learnings/YYYY-MM-DD-<topic>.md`。このリポジトリ固有の実装文脈と結びつけて残す
  2. **Obsidian vault** — `$HOME/develop/obsidian/02_skillup/learn/YYYY-MM-DD-<topic>.md`。プロジェクト横断の個人学習ログとして vault に集約する
...
7. 実装解説 + 技術解説 + **ゼロベース用語解説** + **ハーネス観点** + 判断背景を統合し、選択した保存先の `YYYY-MM-DD-<topic>.md` に出力する
...
保存先: 「出力先の選択」で決めたベースパスの `YYYY-MM-DD-number.md`（ディレクトリがなければ作成）
- プロジェクト選択時: `docs/learnings/YYYY-MM-DD-number.md`
- Obsidian vault 選択時: `$HOME/develop/obsidian/02_skillup/learn/YYYY-MM-DD-number.md`
