...
- 非自明なタスクを完了した際、そこで使ったエンジニアリング技術・設計パターン・ライブラリ機能・アルゴリズム・OS 機能等があれば、応答末尾に「技術メモ」として簡潔に共有すること。完了報告だけで終わらせない。
- 内容: (1) 使った技術・パターンの名称、(2) 何をするものかを 1〜2 文、(3) なぜそれを選んだか・代替案との比較（あれば）、(4) 落とし穴や注意点（あれば）。
...
- より深く学習教材を残したい場合は `/learn` skill を invoke する（`docs/learnings/` に詳細出力）。技術メモは「インライン要約」、`/learn` は「永続学習材料」と棲み分ける。
- 分量: 技術メモは応答の末尾 3〜10 行程度。長文の講義は禁止。技術的回答の本筋を妨げない位置に置く。
