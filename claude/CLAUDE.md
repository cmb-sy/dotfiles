...
- ユーザーが日本語で話しかけたら日本語で、英語で話しかけたら英語で返すこと（使用言語にミラーリングする）。日本語と英語が混在する入力では日本語を優先する。
- 入力の行頭に音声入力マーカー（`🎤` または `音声:`）がある場合、入力言語に関わらず日本語で返すこと（音声入力は文字起こしテキストとして届き判別できないため、ユーザーがマーカーで明示する運用）。
...
- 日本語入力・混在入力には適用しない（既存「言語」セクションの言語ミラーリング原則に従う）。
