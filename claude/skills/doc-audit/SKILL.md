...
name: doc-audit
description: >-
  プロジェクト全体または特定コミット範囲のドキュメントを棚卸ししたいときに使う監査スキル。
  4 Layer 構造で md の陳腐化・欠落・矛盾・depends-on 不整合・未文書化知識を検出し、ユーザー承認後に修正する。
  特定コード変更の影響追従のみが目的なら doc-check を使う。フラグ（--deps / --hygiene / --coverage / --knowledge / --meta / --range / --full / --swarm）の詳細は本文のインターフェースを参照。
user-invocable: true
---
