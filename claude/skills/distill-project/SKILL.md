...
description: >-
  ...
  コードを読んで解釈をまとめたいときに使う。作業しているリポジトリで実行する。
...
**セッション記録用（この会話を読む）:**
過去のログファイルは読まない。**この会話で起きたこと**を書く。

- 何を依頼され、何を作ったか
- どのファイルをどう変えたか（`path:line`）
- どこで判断が分かれ、なぜその方を選んだか
- 何が失敗し、どう気づいて直したか（**ここが最も学習価値が高い**）

時刻・セッション ID・作業時間は書かない（読者像の文体ルール 8）。
...
5. **なぜの連鎖**: 主要な判断について「なぜそうしたか」の次に「なぜその理由が
