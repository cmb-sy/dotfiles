...
```
99_distill/プロジェクト/<リポジトリ名>/
├── 概要.md                      プロジェクト全体の説明（初見の人が最初に読む）
└── <ブランチ名>/
    ├── 概要.md                  このブランチで何が違うか（任意）
    └── <YYYY-MM-DD-HHMM>.md     このセッションの記録（1 セッション 1 ファイル）
```

### 概要は 2 階層ある。混ぜない

**寿命が違う。** ブランチはマージされて消えるが、プロジェクトの説明は残る。
同じ場所に置くと、ブランチごとに重複して内容が乖離していく。

| 置き場所 | 何を書くか | 読者 |
|---|---|---|
| `<リポジトリ>/概要.md` | **このリポジトリは何をするものか。** 目的・要件・設計の骨格・用語。ブランチに依存しない | 初見の人。ここに着地して最初に読む |
| `<ブランチ>/概要.md` | **このブランチで何を変えているか。** main との差分、なぜこのブランチを切ったか | そのブランチの記録を読む人 |

**プロジェクト階層の 概要.md を最優先で作る。** これが無いとプロジェクト
ページに説明が出ず、初見の人は「何のプロジェクトか」を知る前にブランチを
選ばされる。ブランチという概念自体が初心者には未知なので、そこで理解が止まる。

ブランチ階層の 概要.md は**差分を書くことがあるときだけ**作る。main で
作業しているなら不要（プロジェクト階層と重複するため）。
...
EXISTING=$(find "$DEST" -maxdepth 1 -name '*.md' ! -name '概要.md' \
...
### 2. 既存を確認する
```bash
ls "$VAULT/プロジェクト/$REPO/概要.md" 2>/dev/null        # プロジェクトの説明
ls "$VAULT/プロジェクト/$REPO/$BRANCH/" 2>/dev/null       # ブランチの中身
```
...
$VAULT/プロジェクト/$REPO/$BRANCH/概要.md          # 差分があるときだけ
$VAULT/プロジェクト/$REPO/$BRANCH/$STAMP.md        # 1 セッション 1 ファイル（再実行は書き直し）
```
