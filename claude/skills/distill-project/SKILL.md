...
**同じセッションかどうかの判定:** この会話の中で自分が既に実行していれば、
そのときのファイル名を覚えているのでそれを使う。分からなくなった場合（会話が
長くなって前半が要約された等）は新しいファイルを作る。**壊れる方向ではなく
ファイルが 1 つ増えるだけ**なので、迷ったら新規にしてよい。
...
- **この会話で初めて実行する場合** — `STAMP=$(date '+%Y-%m-%d-%H%M')` で新規に取る
- **この会話で既に実行している場合** — そのときに使った `STAMP` をそのまま使う。
  `date` を叩き直さない（叩くと別ファイルになり、重複が生まれる）
