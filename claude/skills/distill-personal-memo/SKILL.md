---
name: distill-personal-memo
description: >-
  実装完了直後に技術・手順を教材として残したいとき、および「もっと根本から」
  「全然わからない」「用語から説明して」と前提を落とした説明を求められたときに使う
  （feature-dev Execute 完了後は確認プロンプトなしで自動実行、手動は /distill-personal-memo）。
  専門用語・手順は「そもそも何か」から細かく解説し、図・表・補足を多用して
  理解しやすさを最大化する。外部の挙動に関する主張は一次情報と照合して出典を付ける。教材は Obsidian の 99_distill/学習メモ/ に書き、
...
確認なしで自動実行。手動は `/distill-personal-memo`。
...
**手動指定（`/distill-personal-memo` の invoke、「教材を出して」等）:** 上記と**同じフロー**。自動と同様、**元データは必ず直前・直近の実装内容のみ**とする（この会話で直前に完了した実装・編集したファイル・行った判断）。手動だからといって commit 履歴や別セッションに切り替えない。
...
`$HOME/develop/obsidian/99_distill/学習メモ/<タイトル>.md` に **Write で書く**。
...
4. `99_distill/学習メモ/` の既存教材と重複しないようトピックを調整する
5. メイントピックおよび Step 2.3 の各用語について、context7 / WebSearch で一次情報を補強する（用語の定義を推測で書かない）
6. 実装解説 + 技術解説 + **ゼロベース用語解説** + 判断背景を統合し、`99_distill/学習メモ/<タイトル>.md`（読みやすい日本語タイトル）に出力する
...
`$HOME/develop/obsidian/99_distill/学習メモ/` のファイル一覧を確認し、同一トピックの重複を避ける。既存と同テーマなら別角度のテーマを選ぶか追補方針を決める。
...
```bash
"$DISTILL" url "$HOME/develop/obsidian/99_distill/学習メモ/<タイトル>.md"
```
