...
利用実績と独立に、スキル内容が**現在も正しく動く前提の上にあるか**を検査する。典型的な壊れ方は、外部サービスの終了で連携フェーズが死ぬ、vault 再編でパスが陳腐化する、参照 URL が stale になる、上位スキルからの導線が無く素通りされる、の 4 系統。
...
5. ハーネス新機能との突合: `99_distill/情報収集/watch/` の直近ダイジェスト（あれば）から Claude Code の新機能を読み、スキルの手順が**新機能で簡素化・置換できる**場合は改善候補として挙げる
...
- `MERGE`: (1) 統合先 SKILL.md に吸収元の固有機能を**オプション/引数/モードとして**追記するドラフトを提示、(2) 承認後に統合先を編集、(3) 吸収元を DELETE と同様に削除、(4) 統合先の description が吸収した機能を**能力として**言い表しているか確認する（旧スキル名や統合の経緯は書かない。SKILL.md に履歴・経緯・自己言及を残さないこと）
