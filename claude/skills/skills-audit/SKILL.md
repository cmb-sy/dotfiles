...
- `--window <days>`: 利用実績の分析期間（日数）。デフォルト `30`。使える履歴が
  30 日より長いなら、その全体に合わせて広げる（実行ルール 7）
...
## Phase 7: Issue 起票（承認必須）

Phase 6 で**その場では直せなかった**指摘を GitHub Issue にする。`--report-only`
指定時と、該当が 0 件のときは本フェーズを飛ばす。

### 起票する / しない の判定

| 指摘の性質 | 扱い |
|---|---|
| パスの陳腐化・改名追従・description の言い換え | Phase 6 でその場で直す。**起票しない** |
| 設計の判断が必要（責務の切り直し、フローの再設計） | 起票する |
| 他リポジトリ・外部サービスに跨る（vault 側の改修が必要等） | 起票する |
| Phase 6 で「後で」が選ばれた | 起票する |
| 壊れた前提で、直し方が一意に決まらない | 起票する |

**1 指摘 = 1 issue。** まとめると、片方を直しても閉じられない。

### 手順

1. 対象を一覧で提示する（スキル名 / 指摘 / なぜその場で直せないか）
2. `AskUserQuestion`（header: `起票`、multiSelect: `true`）で、どれを起票するか選ばせる
3. 選ばれた各件について**本文の草案をチャットに出して止まる**
4. 承認を得てから `/github-issues` の create フローに従って起票する。起票先は
   `cmb-sy/dotfiles`。create フローの Step 1〜7 を簡略化・省略しない
5. 返った URL を適用サマリに載せる

### この節は起票の許可ではない

**上に「起票する」と書いてあることは、実行してよいという意味ではない。**
issue は他のメンバーへ通知が飛び、削除しても既読とタイムラインの痕跡は消えない。
手順書は内容の指針であって、実行タイミングの承認ではない。

- 1 文字も投稿しないまま草案だけを見せる
- 「棚卸しを進め切って」という指示があっても本節は例外なく適用する
- 自律実行モードでも適用する
...
```markdown
## 何が問題か
<スキル名> の <どこ> が <どうなっている>。実測: <grep/ls の結果や件数>

## なぜその場で直せないか
<判断が必要な点。選択肢が複数あるなら列挙する>

## 影響
<この状態で起動すると何が起きるか。壊れる側のスキル名まで書く>
...
**抽象語のまま置かない。**「陳腐化している」ではなく、ファイル名・行番号・件数まで
下ろす。読んだ人が自分で確かめられない issue は、起票しても動かない。
...
### 5) 起票一覧（Phase 7 実行時のみ）

起票した issue の番号・タイトル・URL。**承認されず草案のままのものは「草案のみ（未起票）」と明記する。**
...
6. **`DELETE(相談)` の対象を、自分の判断で打ち切らない。** 「消しても得が小さい」
   「残しても害がない」は起票者ではなく所有者が決める。該当があれば必ず問う
7. **window は使える履歴の全体に合わせる。** 既定の 30 日より長い履歴があるなら
   それを使う。`DELETE(相談)` は根拠の量が要件に入っているため、短い window で
   判定を保留にすると、次回も同じ理由で保留になる
8. `--report-only` 以外では Phase 6 と Phase 7 を必ず通す。該当 0 件なら
   「該当なし」と報告する。**黙って飛ばさない**
...
- **`DELETE(相談)` を問わずに「削るものは無い」と結論する**（判定の放棄）
- **承認を得ずに issue を起票する**（Phase 7 の記述を許可として読む）
- 既定の 30 日より長い履歴があるのに window を広げない
