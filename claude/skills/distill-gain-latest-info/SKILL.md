...
**基準日:** `since` はスコープごとに独立して求める。`99_distill/情報収集/watch/` を新しい順に走査し、実行対象スコープの見出し（例: `## GitHub`）に内容がある最新ダイジェストの `date` を採用する（無ければ 14 日窓）。スコープ単位で分離することで、`--only` の部分実行が基準日を共有し後続スコープを取りこぼす事態を防ぐ。
...
出力: `99_distill/情報収集/research/YYYY-MM-DD-<topic>.md`（`<topic>` はスラグ化する: 小文字化し、空白はハイフンに置換、`/`・`:` 等のパス非安全文字は除去/置換する）
