...
- **verification**:
  ...
  2. 各 finding に対応する設計書の変更を `git log -p -- docs/superpowers/specs/*-design.md` または設計書本文の該当記述で照合する
...
- **fail_diagnosis_hint**: 不一致箇所の両方の記述を並べ、`git log -p -- docs/superpowers/specs/*-design.md` で直近のレビュー修正コミットからどちらが新しい意図かを追跡する
- **depends_on_artifacts**: [docs/superpowers/specs/*-design.md]

### D2-04: 修正後の設計書が git commit 済みで handover が実行されている
- **severity**: blocker
- **verify_type**: automated
- **verification**:
  1. `git status --porcelain -- docs/superpowers/specs/*-design.md` を実行し、設計書が未コミット変更リストに含まれないことを確認する
...
- **depends_on_artifacts**: [docs/superpowers/specs/*-design.md, .agents/handover/]
