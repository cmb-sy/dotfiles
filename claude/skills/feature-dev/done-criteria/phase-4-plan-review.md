...
- **verification**:
  ...
  2. 各 finding に対応する計画書の変更を `git log -p -- docs/superpowers/plans/*-plan.md` または計画書本文の該当記述で照合する
  3. 対応する変更が見つからない finding をリストアップする
  4. 却下された finding には却下理由が記録されているか確認する
- **pass_condition**: 手順3のリストが0件、かつ却下理由なしの却下 finding が0件
- **fail_diagnosis_hint**: 未反映の finding ID を特定し、計画書の該当タスクを確認する。テストケース定義への波及が必要な finding は `docs/superpowers/plans/*-test-cases.md` 側の更新漏れも確認する
- **depends_on_artifacts**: [artifacts/reviews/, docs/superpowers/plans/*-plan.md]

### D4-03: Evidence Plan 再評価の要否判定が記録されている
- **severity**: quality
- **verify_type**: automated
- **verification**:
  1. `Glob("docs/superpowers/plans/*evidence-plan.md")` で Evidence Plan を検索し、その初回コミットを `git log --format=%H -- <evidence-plan パス>` の最終行で特定する（Phase 1 Audit Gate で生成されたコミット）
  2. `git log --oneline <初回コミット>..HEAD -- docs/superpowers/specs/*-design.md` で、Evidence Plan 生成以降の設計書変更コミット件数を数える
...
- **depends_on_artifacts**: [docs/superpowers/specs/*-design.md, docs/superpowers/plans/*evidence-plan.md]

### D4-04: 修正後の計画書が git commit 済みで handover が実行されている
- **severity**: blocker
- **verify_type**: automated
- **verification**:
  1. `git status --porcelain -- docs/superpowers/plans/` を実行し、計画書とテストケース定義が未コミット変更リストに含まれないことを確認する
...
- **depends_on_artifacts**: [docs/superpowers/plans/*-plan.md, .agents/handover/]
