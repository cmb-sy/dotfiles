...
- **verification**:
  1. `Grep("^- \\[ \\]", "docs/superpowers/plans/*-plan.md")` で未完了 checkbox のタスク行を抽出する
  2. `Grep("^- \\[x\\]", "docs/superpowers/plans/*-plan.md")` で完了 checkbox のタスク行を抽出し、1件以上あるか確認する
...
- **depends_on_artifacts**: [docs/superpowers/plans/*-plan.md, tests/]
...
- **verify_type**: automated
- **verification**:
  1. `Glob("docs/superpowers/plans/*evidence-plan.md")` で Evidence Plan を検索し、その初回コミットを `git log --format=%H -- <evidence-plan パス>` の最終行で特定する
...
- **depends_on_artifacts**: [docs/superpowers/plans/*-plan.md, src/]
