...
- **verification**:
  1. Layer 1 の検出結果を次のいずれかから取得する: (a) `Glob("artifacts/doc-audit/phase-6-script-output.json")` で見つかる検出結果ファイル、または (b) フェーズ実行ログに記録された `doc-audit.sh --json` の JSON 出力。doc-audit.sh は JSON を標準出力に書くため、(a) が無くても (b) があれば有効な証跡として扱う
...
- **depends_on_artifacts**: [artifacts/doc-audit/phase-6-script-output.json, docs/]
...
- **depends_on_artifacts**: [docs/, artifacts/doc-audit/phase-6-script-output.json]
