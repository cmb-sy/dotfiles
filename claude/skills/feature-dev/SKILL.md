...
name: feature-dev
description: >-
  feature spec（何を作るか + なぜ作るか）から merge-ready なコードまでを、
  設計→レビュー→計画→レビュー→実装→レビュー→統合の10フェーズで品質ゲート付きに完遂したいときに使う開発オーケストレーター。
  新機能開発を一気通貫で進めたい場合に起動する。フラグ（--codex / --e2e / --smoke / --doc / --iterations / --swarm）の詳細は本文の Options を参照。
user-invocable: true
---
...
`--swarm` 有効時、inspection 基準を含む Phase 1-9 の Audit Gate をエージェントチーム（3メンバー: automated-verifier, inspection-verifier, evidence-verifier）で実行する。メンバー間で findings を相互検証し、合意した verdict を返却する。Phase 10 は `audit: lite` のため Audit Team を使わない（下記 Audit Gate Lite）。詳細は `../_shared/audit-gate-protocol.md` セクション 10 を参照。
...
レビューフェーズ（Phase 2, 4, 8, 9）完了後は **必ず** `/handover` を実行する。他フェーズでは、context 逼迫時（下記参照）を除き実行不要。
