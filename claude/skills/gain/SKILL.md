...
- `--target X` は選択スコープ内の対象を X に限定する（例: `--only sns --target <engineer>` は当該エンジニアの SNS のみ観測。github なら X はそのスコープ内のリポジトリ/ハンドル）
...
- 選んだアドホック対象が `sources.yaml` に未登録なら、実行後に **「今後も watch 対象に追加しますか?」** を AskUserQuestion で確認 → YES で該当セクションに追記（単発 → 永続化）。ユーザー承認なしに追記しない。この確認は watch スコープのアドホック対象にのみ適用し、research のトピックには行わない（research トピックは `sources.yaml` の watch 対象ではない）
...
**基準日:** `since` はスコープごとに独立して求める。`02_skillup/情報収集/watch/` を新しい順に走査し、実行対象スコープの見出し（例: `## GitHub`）に内容がある最新ダイジェストの `date` を採用する（無ければ 14 日窓）。スコープ単位で分離することで、`--only` の部分実行が基準日を共有し後続スコープを取りこぼす事態を防ぐ。
...
**既出照合（全スコープ共通）:** services に限らず、news / sns / engineers(blog) も直近 3 件のダイジェストの URL・タイトルと突合し、既出エントリはダイジェストから除外する（WebSearch は基準日で絞れないため、これが実質の差分化手段）。

**日付の不確実性:** WebSearch 系（news / sns）は取得結果に日付があれば基準日でフィルタし、日付を特定できないエントリは掲載してよいが **「日付不明」を明記**する。日付不明のまま「最新」と断定しない。

**同日再実行:** `watch/YYYY-MM-DD.md` が既に存在する場合は新規作成せず、同一ファイルの該当スコープ見出しに追記する（peers と同じ扱い）。
...
出力: `02_skillup/情報収集/research/YYYY-MM-DD-<topic>.md`（`<topic>` はスラグ化する: 小文字化し、空白はハイフンに置換、`/`・`:` 等のパス非安全文字は除去/置換する）
...
ダイジェスト md（Obsidian の callout・表・Mermaid を活用した以下の構造で書く）:

**全スコープ共通の深さ原則:** 「何が起きたか（事実）」ではなく **「何がどう変わったか + なぜ重要か」** を主役にする。事実の翻訳・列挙で終わらせず、従来との差分・変更前後の対比・自分の運用への含意まで踏み込む。リリースノートやニュース見出しをそのまま訳しただけの項目は不可。
...
scopes: [github, services, engineers, news]
...
### owner/repo — <テーマ> (vX.Y.Z)

> [!tip] 自分への関係 [🆕 NEW]
> 実 grep に基づく判定と、自分の運用への含意 2〜4 文。

**何をしているか**（2〜4文。専門用語は「そもそも何か」から解説。プロジェクトの位置づけ）

**何が変わったか**（このダイジェストの主役・4〜8文）: 事実の列挙で終わらせず、**「以前はどうで、今回どう変わり、その結果何ができるようになったか」**を書く。
- **差分**: 前バージョン / 従来のやり方と比べて具体的に何が変わったか（新機能・仕様変更・非推奨化・破壊的変更）。変更前→変更後を対比で示す
- **なぜ重要か**: その変更がユーザー（＝自分の運用）にとってなぜ効くか。放置すると何を取りこぼすか
- **深さの基準**: 「◯◯が追加された」だけは不可。「◯◯が追加され、これまで△△で回避していた□□が不要になった」まで踏み込む。リリースノートの箇条書きを訳しただけにしない

**仕組み**（構造・フローが3要素以上絡む場合のみ Mermaid か表で図解。無理に描かない）

**使ってみるなら**（github スコープは必須）: 実際に使う場合のコマンド・設定・ワークフロー例をコードブロックで示す。収集データ（README/リリースノート/ADR）由来の事実と、そこからの推測（「推定」と明記）を区別する。

**source**: <URL>

## サービス更新

（サービスごとに `### 名前` + エントリ表 `| 版/日付 | 変更 | 自分への影響 |` + 要対応事項は callout）

## エンジニア発信（blog / SNS·信頼度低）

（人物ごとに記事表 `| 日付 | タイトル | 要旨 |` + 特に重要な 1〜2 本を callout で深掘り）
...
**callout の使い分け（関連性ラベルと対応させる）:**

| ラベル | callout |
|---|---|
| 🆕 NEW（取り込む価値） | `> [!tip]` |
| 🔁 OUTDATED（自分が遅れている/要更新） | `> [!warning]` |
| ✅ COVERED（対応済み） | `> [!success]` |
| ❌ N/A（対象外） | `> [!quote]` |
| セキュリティ・障害系の注意情報 | `> [!danger]` |

**深さの基準:** 1 item あたり 10〜20 行を目安に、「何をしているか」は前提概念から書く（CLAUDE.md 技術メモと同じ教育深度）。比較対象が3つ以上あるときは表を必須とする。Mermaid 図は構造・フローの理解を速めるときだけ使い、装飾目的では使わない。

- `[[]]` リンク・`#tag` を維持。実行後 `index.md` に新ファイルへの `[[watch/YYYY-MM-DD]]` / `[[research/YYYY-MM-DD-<topic>]]` 形式のリンクを追記（`index.md` が無ければ「# 情報収集 MOC」見出しで作成。既に同名リンクがあれば追記しない）
...
| スコープのソース定義が空（`[]`） | 「<scope>: ソース未登録のためスキップ」と 1 行報告して次スコープへ（エラー扱いしない） |
