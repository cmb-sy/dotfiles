...
@test "services の取得ルールが watch の形で分かれている" {
  grep -qF 'watch: diff' "$SK"
  grep -qF 'gain-state snapshot' "$SK"
}

@test "形の合わない監視先を既定ルールで取らない旨が Red Flags にある" {
  grep -qF '日付つきエントリを持たないページを changelog として扱う' "$SK"
}

# --- sources モードの穴 ---

@test "sources は list の終了ステータスを見る" {
  # ディレクトリや FIFO を渡されたとき、gain-state は非 0 で止まる。
  # 行数だけ見ると、誤った表をそのまま見せる。
