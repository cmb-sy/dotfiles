...
[OCI VM.Standard.A1.Flex  2 OCPU / 12GB RAM / Ubuntu 24.04 (aarch64)]
   ...
   └─ Claude Code + gh  long-lived token / gh auth で認証
```
...
4. **Claude サブスクリプション** — `claude setup-token` で long-lived token を発行できること
...
2. **Claude Code** — インストール後、`claude setup-token` が対話で起動する。
   ブラウザの無い環境のため、表示された URL を Mac 側のブラウザで開いて認証コードを貼り戻す
3. **gh** — `gh auth login` が対話で起動する。`HTTPS` + `Login with a web browser` を選び、
   ワンタイムコードを Mac 側ブラウザで入力する
4. **systemd user units** — `tmux.service` / `keepalive.timer` を有効化し、
   `loginctl enable-linger` で SSH 切断後もユニットが動き続けるようにする
5. **CLAUDE.md** — グローバル CLAUDE.md を `~/.claude/` に symlink
...
- 認証（Tailscale auth key / claude token / gh auth）は全て再発行可能。バックアップ対象にしない
